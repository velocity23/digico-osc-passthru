...
func (s *StandardDispatcher) Dispatch(packet Packet, remoteAddr net.Addr) {
	...
	case *Message:
		for addr, handler := range s.handlers {
			if p.Match(addr) {
				handler.HandleMessage(p, &remoteAddr)
			}
		}
		if s.defaultHandler != nil {
			s.defaultHandler.HandleMessage(p, &remoteAddr)
		}
			...
			for _, message := range p.Messages {
				for address, handler := range s.handlers {
					if message.Match(address) {
						handler.HandleMessage(message, &remoteAddr)
					}
				}
				if s.defaultHandler != nil {
					s.defaultHandler.HandleMessage(message, &remoteAddr)
				}
			}

			// Process all bundles
			for _, b := range p.Bundles {
				s.Dispatch(b, remoteAddr)
			}
		}()
	}
}
